...
import {Button,TextField, Typography, Modal, Box, useTheme,IconButton } from '@mui/material';
import { editTemplate } from 'utils';
...
export default function MessageTemplateModal({temp, setMessageTemplates, messageTemplates}) {
  const theme = useTheme();
  const [open, setOpen] = React.useState(false);
  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);
  const [formValues, setFormValues] = useState(temp);
  ...
  async function handleSave(){
    let res = await editTemplate(formValues._id, formValues);
    if(res.status === 200){
      let templates = messageTemplates.map((template)=> {
          if(template._id === formValues._id){
            return formValues;
          }else{
            return template;
          }
  ...
  return (
    <div>
     <IconButton><FiEdit onClick={handleOpen} className='cursor-pointer m-2' size="1.5rem" /></IconButton >
      <Modal
        open={open}
        onClose={handleClose}
        ...
        <Box sx={style}>
          ...
          <Box sx={{display: "flex", flexDirection: "row", flexWrap: "wrap", marginTop: 2}}>
            <Box sx={{width: "47%", margin: 1}} >
              ...
              <TextField value={formValues._id} readOnly />
            </Box>
            <Box sx={{width: "47%", margin: 1}} >
              <Typography>
                Name
            ...
            <TextField value={formValues.createdAt} onChange={handleFormValueChange} name="text" />
            </Box>
            <Box sx={{width: "47%", margin: 1}} >
            <Typography>
              Created By
            </Typography>
            <TextField value={formValues.createdBy} onChange={handleFormValueChange} name="text" />
            </Box>
          </Box>
